...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        # 根据类别信息初始化空列表，整个db_infos为字典，每个类别下为整个列表
        ...
        for db_info_path in sampler_cfg.DB_INFO_PATH:
            ...
            'score': -1.0}
            """
            # pickle.load(f)读取pkl文件，这样看来db_info_path实际上是原pkl文件中的某个样本
            # 结果就是将目标点云读取到数据库 db_infos中
            # 每个key对应的infos都是多个info的列表，每个info是上面的那串数据
            with open(str(db_info_path), 'rb') as f:
                infos = pickle.load(f)
                [self.db_infos[cur_class].extend(infos[cur_class]) for cur_class in class_names]
        # 上述代码最后结果就是将生成的点云目标按照gt_sample将其存入到db_infos数据库中

        # 预处理，滤除点数少和识别困难的点云
        # sampler_cfg.PREPARE: filter_by_min_points:,filter_by_difficulty
    ...
    def filter_by_difficulty(self, db_infos, removed_difficulty):
        ...
        """
        # 采样数据库db_infos 有三个键，分别是相应的类别，然后每个键后的value是所有的采样数据
        # 仍用一个个字典表示
        new_db_infos = {}

        # key是类别，dinfos是该类别下所有的gt_sample
        for key, dinfos in db_infos.items():
            pre_len = len(dinfos)
            new_db_infos[key] = [
                # 若info这个点云数据的difficulty不在去除的里面，则加入到新的数据库中
    ...
    def filter_by_min_points(self, db_infos, min_gt_points_list):
        ...
        for name_num in min_gt_points_list:  # ['Car:5', 'Pedestrian:5', 'Cyclist:5']
            ...
            if min_num > 0 and name in db_infos.keys():
                filtered_infos = []
                # 将点云大于设定点云数量最小值的info 保存在filtered_infos中，作为某个类别的新数据库
