...
def random_flip_along_x(gt_boxes, points):
    ...
    Args:
        ...
        points: (M, 3 + C)
        7: (x, y, z, w, l, h, θ)；x, y, z预测了目标中心点到该anchor左上顶点的偏移数值,
            w，l，h预测了基于该anchor长宽高的调整系数，θ预测了box的旋转角度,
            方向类别预测了box的朝向,
    Returns:
    """
    # 随机选择是否翻转
...
def global_rotation(gt_boxes, points, rot_range):
    ...
    # 沿z轴旋转noise_rotation弧度，这里之所以取第0个，是因为我们补了一维Batch为1，是因为rotate_points_along_z对batch进行处理，而这里仅处理单个点云
    # 因为rotate_points_along_z对batch作用，所以得对原始的points 和gt_boxes补出第三维points[np.newaxis, :, :]
