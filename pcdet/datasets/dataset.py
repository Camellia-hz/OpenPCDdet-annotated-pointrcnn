...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, logger=None):
        ...
        # 日志
        self.logger = logger
        # 数据集根目录。如果输入了，则是输入，否则就使用kitti_dataset.yaml文件中的data_path
    ...
    def prepare_data(self, data_dict):
        ...
        if self.training and len(data_dict['gt_boxes']) == 0:
            """
                如果处于训练模式，并且数据中不含有gt_boxes
                首先，在数据长度范围内产生一个随机数
                然后调用__getitem__方法获取该索引的数据字典
                就是如果当前的这个点云里面一个物体都没有，需要重新在get一个
            """
    ...
    def collate_batch(batch_list, _unused=False):
       ...
       假如说 一个batch 多个点云，但是和二维图像尺寸一致不一样，每个点云的gt不同，数据维度不同
       所以，需要将一个bs下所有点云的gt框给拼接起来，变成一个大的bs=1
       ...
       """
        ...
        for key, val in data_dict.items():
            try:
                ...
                # voxel_coords: optional (num_voxels, 3)
                # voxel_num_points: optional (num_voxels)
                # 将不同点云中的pillar拼接起来，voxels and voxel_num_points
                if key in ['voxels', 'voxel_num_points']:
                    # 按照第0维 也就是bs维拼接，第0维就没了，
                    # (bs, num_voxels, max_points_per_voxel, 3 + C)-->(bs*num_voxels, max_points_per_voxel, 3 + C)
                ...
                # 对于 point
                    ...
                    # 构造空的box3d矩阵（B, N, 7）,其中N为bs中某个具有最多gtbox的点云的框的数量
                    # 保证所有点云的框都能被填充到box3d矩阵中
                    batch_gt_boxes3d = np.zeros((batch_size, max_gt, val[0].shape[-1]), dtype=np.float32)
                    # val[k]为第k个点云的box矩阵 (N, 7)
