...
class AnchorGenerator(object):
    ...
    def generate_anchors(self, grid_sizes):
        ...
        # 2.三个类别的先验框逐类别生成
        # 生成anchor需要在特征图的每个像素点上生成若干个不同尺寸的anchor
        # anchor_sizes [[3.9, 1.6, 1.56]],--->whl
        # #     'anchor_rotations': [0, 1.57],
        # #     'anchor_bottom_heights': [-1.78],
        for grid_size, anchor_size, anchor_rotation, anchor_height, align_center in zip(
                grid_sizes, self.anchor_sizes, self.anchor_rotations, self.anchor_heights, self.align_center):
            # 2 * 3 * 1，每个点返回6个anchor 每个类别两个框
            # len(anchor_rotation) * len(anchor_size) * len(anchor_height)
            # 不同的角度、不同的size、不同的高度生成锚框
            # 角度有两个，一个0， 一个90度， size3个，height也一个，
            # 这些都是初始的先验框，其中的参数可能不准确，会经过训练，结合gt，训练出回归参数
            num_anchors_per_location.append(len(anchor_rotation) * len(anchor_size) * len(anchor_height))
            # 　不需要对齐中心点来生成先验框
            if align_center:
                # anchor_range = point_cloud_range[0, -39.68, -3, 69.12, 39.68, 1] grid_size此时为feature map size 为list
                # 它的大小为3个[216, 248] x_stride相当于216个x格子，每一个格子有多长
                x_stride = (self.anchor_range[3] - self.anchor_range[0]) / grid_size[0]
                y_stride = (self.anchor_range[4] - self.anchor_range[1]) / grid_size[1]
                # 中心对齐，平移半个网格 x_offset如果中心对齐，则得求出半个网格有多长
            ...
            # 产生x坐标 --> 216个点 [0~~69.12], 每个点间隔step: x_stride
            # 每个网格生成一个坐标，作为anchor的x坐标
            ...
            # x_shifts(216,248,1)--->x_shifts在第一维度，每一列都由x_shifts原数据填充，共y_shifts列
            # 同理，y_shifts 的每一行都由y_shifts原数据填充，共x_shifts行
            # meshgrid可以理解为在原来的维度上进行扩展,例如:
            # x原来为（216，）-->（216，1, 1）--> (216,248,1)
            # y原来为（248，）--> (1，248，1）--> (216,248,1)
            # z原来为 (1, )  --> (1,1,1)    --> (216,248,1)

            # anchor各个维度堆叠组合，生成最终anchor(1,432,496,1,2,7）
            # 2.4.1.堆叠anchor的位置 
            # [x, y, z, 3]-->[216, 248, 1, 3] 代表了每个anchor的位置信息
            # 其中3为该点所在映射tensor中的（z, y, x）数值
            # [x, y, z, 3]，([216, 248, 1, 3]) 最后的3分别为zyx
            ...
            # 'anchor_sizes': [[3.9, 1.6, 1.56]], anchor_size.shape[0]
            anchors = anchors[:, :, :, None, :].repeat(1, 1, 1, anchor_size.shape[0], 1)

            # (1, 1, 1, 1, 3) --> (216, 248, 1, 1, 3)  anchor_size最后一维为3，是因为[[3.9, 1.6, 1.56]]
            ...
            # 2.5 置换anchor的维度
            # [x, y, z, num_anchor_zie, num_rot, 7]-->[z, y, x, num_anchor_size, num_rot, 7]
            ...
            # (1，248，216，1，2，7）--->BS, (feature map: 248，216，1), 每个点两个锚框, 每个锚框的7个属性
