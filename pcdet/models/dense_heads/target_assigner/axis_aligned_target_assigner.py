...
class AxisAlignedTargetAssigner(object):
    def __init__(self, model_cfg, class_names, box_coder, match_height=False):
        ...
        # self.box_coder = box_coder  传入的box_coder是一个方法（函数）pcdet.utils.box_coder_utils.ResidualCoder
    ...
    def assign_targets(self, all_anchors, gt_boxes_with_classes):
        ...
        Args:
            all_anchors: [(N, 7), ...] all_anchors-->list:3 [(1，248，216，1，2，7），(1，248，216，1，2，7），(1，248，216，1，2，7)]
        ...
        for k in range(batch_size):
            ...
            """
            # 上面的意思是gt_boxes_with_classes: (B, M, 8) ，对于每个batch数据，M为每个batch下的gt数量，这里的M是拥有最多的gt数量的batch决定的
            # 其他的若不够M个，就需要补0
            ...
            # all_anchors all_anchors-->list:3 [(1，248，216，1，2，7），(1，248，216，1，2，7），(1，248，216，1，2，7)]
            for anchor_class_name, anchors in zip(self.anchor_class_names, all_anchors):
                # anchor_class_name : 车 | 行人 | 自行车
                # anchors : (1, 200, 176, 1, 2, 7)  7 --> (x, y, z, l, w, h, θ)
                # 下面这段代码是从gt的类别中，找到与当前anchor同一类别的（因为anchor是分类别生成的）所有gt，
                # 根据cur_gt_classes生成的mask掩码，然后根据掩码从cur_gt找到gt属性
                ...
                if self.use_multihead:  # False
                    ...
                    # List: 根据label_mask索引得到该帧中当前需要处理的类别  --> 车 | 行人 | 自行车
                ...
                single_target = self.assign_targets_single(
                    anchors,  # 该类的所有anchor
                    cur_gt[mask],  # GT_box  shape : （num_of_GT_box, 7） 该类的所有gt
    ...
    def assign_targets_single(self, anchors, gt_boxes, gt_classes, matched_threshold=0.6, unmatched_threshold=0.45):
        ...
        Args:
            ...
            gt_classes: (该帧中该类别的GT数量, 1) 'CAR','CAR'....
        ...
        # 初始化anchor对应的label(该anchor是前景还是背景，若是前景，则类别也确定了，因为anchor本身就是某个类别的)和
        # gt_id(该anchor对应的哪个gt) ，并置为 -1，-1表示loss计算时候不会被考虑，背景的类别被设置为0
        ...
        if len(gt_boxes) > 0 and anchors.shape[0] > 0:
            ...
            # anchor_by_gt_overlap代表当前类别的所有anchor和当前类别中所有GT的iou ，返回二维矩阵，表示当前类别的所有anchor和当前类别中所有GT的iou
            ...
            """
            矩阵比较例子 :
            行是anchor，列是gt，dim=0，表示保留行，从每列取最大值
