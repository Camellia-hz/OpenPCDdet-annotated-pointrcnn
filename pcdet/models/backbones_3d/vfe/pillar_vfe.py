...
class PFNLayer(nn.Module):
    def __init__(self,
        ...
        if self.use_norm:
            ...
            # 通道即是特征，每个点(x,y)处的通道数量，即是特征长度
            # 输出的通道数是64
            # x的维度由（M, 32, 10）升维成了（M, 32, 64）,max pool之后32才去掉
            # 因为，Linear当处理高于二维的数据时，会自动将最后一维做特征维，前面的维度将会被合并在一起看待
            # 使用1*1卷积的效果是一样的
            # 使用conv1d时注意，需要改变数据维度, 操作与norm一致 linear接收数据时，特征维(通道维)在最后一维，但是对于convd，特征维在第二维度
    ...
    def forward(self, inputs):
        # inputs: (M, 32, 10) 由于M是将整个Batch拼接过后得到的结果 BS*（每个点云的pillar数）
        # M>self.part
        if inputs.shape[0] > self.part:
            # nn.Linear performs randomly when batch size is too large
            num_parts = inputs.shape[0] // self.part
            # 相当于将inputs 按照第一个batch维度分块去执行self.linear，最后再将结果进行拼接
        ...
        # 对于BatchNorm2d来说，是对三维图像，维度为（N, C，H, W) - 输出：（N, C, H, W）
        # 但是对于这里inputs（包含batch）是三维，所以用BatchNorm1d: (N, C, L)-->(N, C, L)
        # 因此，执行norm时，先变换维度，执行之后再变回来
        x = self.norm(x.permute(0, 2, 1)).permute(0, 2, 1) if self.use_norm else x
        torch.backends.cudnn.enabled = True
        x = F.relu(x)
        # 完成pointnet的最大池化操作，找出每个pillar中最能代表该pillar的点
        # x_max shape :(M, 1, 64)　
        # max取了每个特征下，值最大的那一个，由于keepdim ，所以输出为M, 32, 64，但是在取一个[0]，保留第一个有效值
        # 变成了 （M, 1, 64）　
        ...
        if self.last_vfe:
            ...
            # repeat的用法,x.repeat(a,b,c) 在第一个维度上repeat a倍, 在第二个维度上repeat b倍, 在第三个维度上repeat c倍
            # 并且重复的维度是从最后一个维度往前
            # inputs.shape[1]是32 其实这里用的就是voxel论文当中vfe的做法，只要不是最后一块，
            # 每次都将生成的特征先进行max，然后重复一下，再拼接到原来的局部特征上
            # x_max.shape:(M, 1, 64) repeat-->(M, 32, 64)
            x_repeat = x_max.repeat(1, inputs.shape[1], 1)
            x_concatenated = torch.cat([x, x_repeat], dim=2)
            # x_concatenated:(M, 32, 128)
...
class PillarVFE(VFETemplate):
    ...
    def __init__(self, model_cfg, num_point_features, voxel_size, point_cloud_range, **kwargs):
        ...
        pfn_layers = []
        # i=0时 已经是最后一个vfe块了 虽然一共就一个，最后一个vfe块是不需要局部全局特征拼接的
        # len(num_filters) - 1 == 1
    ...
    def forward(self, batch_dict, **kwargs):
        """
        在VFE之前的data_dict的数据如下所示：即在dataset.py中最后将整个batch数据进行处理的collate_batch的输出
        batch_dict:
            points:(N,5) --> (batch_index,x,y,z,r) batch_index代表了该点云数据在当前batch中的index, 同一个点云中的index是一致的
        ...
        # 每个pillar中有一个合点，是所有点的坐标的和: (M, 1, 3) ,然后用合点的xyz 除以点的总数，得到平均值：(M, 1, 3/voxel_num_points)
        ...
        #  所有pillar中每个点的x坐标: voxel_features[:, :, 0]
        #  coords[:, 3].to(voxel_features.dtype).unsqueeze(1) * self.voxel_x + self.x_offset
        #  从点云的x初始位置开始，加上当前pillar是在第几个坐标点处，比如是432中的第50，那么pillar最终的x中心是，初始x+50*voxel_x
        ...
        而刚才上面的计算中，会导致这些
        由0填充的数据在计算出现xc,yc,zc和xp,yp,zp数值，
        所以需要将这个被填充的数据的这些数值清0,
        因此使用get_paddings_indicator计算features中哪些是需要被保留真实数据和需要被置0的填充数据
        """
        # 得到mask维度是（M， 32） bool量，对每一个pillar来说，一个32长度的特征向量，指出每一个点是否是填充点
