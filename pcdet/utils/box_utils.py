...
def mask_boxes_outside_range_numpy(boxes, limit_range, min_num_corners=1):
    ...
    if boxes.shape[1] > 7:
        boxes = boxes[:, 0:7]
    # corners为一个box的八个点，根据box的7个量，求出八个角点的坐标，所以为(8, 3)
    corners = boxes_to_corners_3d(boxes)  # (N, 8, 3)
    # all()函数用于判断给定的可迭代参数iterable中的所有元素是否都为TRUE，如果是返回True，否则返回False。
    # 默认从第2维度，也就是(N, 8, 3)中的3: 坐标维度
    # mask为(N, 8) 针对每个box的每个点 都判断是否在点云范围内
    mask = ((corners >= limit_range[0:3]) & (corners <= limit_range[3:6])).all(axis=2)
    # mask.sum(axis=1) 对每个box的八个角点在范围内的数量进行求和，维度为(N, 1) 若大于阈值的，为true
...
def boxes_iou_normal(boxes_a, boxes_b):
    ...
    # 获得左上角坐标 交集左上角应该取两者左上角的较大值，交集右下角应该取两者右下角较小值
