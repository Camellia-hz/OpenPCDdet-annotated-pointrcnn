...
def rotate_points_along_z(points, angle):
    ...
    # 构造旋转矩阵batch个，旋转矩阵是根据角度后固定下来的值，3*3
    cosa = torch.cos(angle)  # 1*B
    sina = torch.sin(angle)
    zeros = angle.new_zeros(points.shape[0])
    ones = angle.new_ones(points.shape[0])
    # 根据旋转角度构造出按照z轴旋转的旋转矩阵，对每个bs的points进行旋转
    # view(-1, 3, 3), 第0维自适应为batch
    rot_matrix = torch.stack((
        cosa, sina, zeros,
        -sina, cosa, zeros,
        zeros, zeros, ones
    ), dim=1).view(-1, 3, 3).float()
    # 对点云坐标进行旋转，每个点云与旋转矩阵相乘
...
def mask_points_by_range(points, limit_range):
    # 根据点云的范围产生mask，过滤点云
    # points中每个点的x, y坐标都在limit_range中
